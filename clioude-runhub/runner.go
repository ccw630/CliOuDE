...
func (c *Runner) readPump() {
	...
	c.hub.register <- RunnerInfo{runner: c, conf: string(message)}
	for {
		_, message, err := c.conn.ReadMessage()
		...
		if bytes.Equal(message, []byte("\x00\xe7")) {
			c.send <- []byte("1\n\xe0")
		}
